...
macOS / Linux / Windows Git Bash에서 설치 스크립트로 최신 GitHub Release 실행 파일을 받을 수 있습니다.
...
```bash
curl -fsSL https://raw.githubusercontent.com/Epsilondelta-ai/pi-web-ui/main/scripts/install.sh | PI_WEB_VERSION=v1.0.0 sh
```

기본 설치 경로는 `~/.local/bin`입니다. 다른 경로에 설치하려면:

```bash
curl -fsSL https://raw.githubusercontent.com/Epsilondelta-ai/pi-web-ui/main/scripts/install.sh | PI_WEB_INSTALL_DIR=/usr/local/bin sh
```
...
`~/.local/bin`이 `PATH`에 없다면 shell profile에 추가하세요.

```bash
export PATH="$HOME/.local/bin:$PATH"
```

Windows에서 shell script를 쓰지 않는 경우에는 Release 페이지에서 `pi-web_1.0.0_windows_amd64.zip` 또는 `pi-web_1.0.0_windows_arm64.zip`을 내려받아 압축 해제 후 `pi-web.exe`를 실행하면 됩니다.
...
설치 스크립트는 현재 OS/CPU를 감지해서 아래 asset 중 하나를 다운로드합니다.
