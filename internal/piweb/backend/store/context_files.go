...
func contextFilesBanner(cwd string) Message {
	paths := loadedContextFilePaths(cwd)
	if len(paths) == 0 {
		return Message{Kind: "banner", Text: "loaded AGENTS.md files · none"}
	}
	lines := []string{fmt.Sprintf("loaded AGENTS.md files · %d", len(paths))}
	for _, path := range paths {
		lines = append(lines, "- "+path)
	}
	return Message{Kind: "banner", Text: strings.Join(lines, "\n")}
}
