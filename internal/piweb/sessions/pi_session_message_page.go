...
func tailSessionMessages(file *os.File, end int64, limit int) ([]shared.Message, string, bool, error) {
	var selected []shared.Message
	var prefix []byte
	oldestOffset := end
	position := end
...
func collectTailMessages(
	data []byte,
	baseOffset int64,
	selected []shared.Message,
	oldestOffset *int64,
	limit int,
) ([]shared.Message, int64) {
	lineEnd := len(data)
