...
func CreatePiSessionFile(cwd string) (shared.Session, string, error) {
...
func ParsePiSessionFile(path string) (ParsedSession, error) {
	...
	for scanner.Scan() {
		...
		case "compaction":
			messages = append(messages, shared.Message{Kind: "pi", Text: fmt.Sprintf("context summarized · %d tokens before compaction", entry.TokensBefore)})
		case "model_change":
			messages = append(messages, shared.Message{Kind: "banner", Text: fmt.Sprintf("model changed · %s/%s", entry.Provider, entry.ModelID)})
		case "thinking_level_change":
			messages = append(messages, shared.Message{Kind: "banner", Text: fmt.Sprintf("thinking level · %s", entry.ThinkingLevel)})
		}
	}
	...
	session := shared.Session{ID: header.ID, Title: title, LastUsed: relTime(stat.ModTime()), Workspace: workspaceIDFromPath(header.CWD)}
