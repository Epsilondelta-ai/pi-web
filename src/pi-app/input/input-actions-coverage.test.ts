...
describe("pi-app input actions coverage", () => {
  ...
  it("dispatches click actions and list navigation", async () => {
    ...
    const calls = [];
    [
      "route", "refreshWorkspaces", "browseFolder", "loadFolder", "openWorkspacePath", "toggleTree", "showFileTreePanel", "showGitHistory", "loadMoreGitHistory", "selectGitCommit", "refreshTree",
    ...
    const actions = [
      ...
      "folder-open-current", "toggle-tree", "show-file-tree", "show-git-history", "load-more-git-history", "select-git-commit", "refresh-tree", "tree-root-menu", "open-file", "close-file-preview",
