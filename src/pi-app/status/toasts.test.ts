...
describe("pi-app toast notifications", () => {
  ...
  it("shows system warnings for update and auth credential failures once", async () => {
    ...
    expect(app.isPiPackageUpdateIgnored("pi-package-update-ignore:global:npm:skip:1.1.0")).toBe(true);
    app.notifyPiPackageUpdateAvailable([
      { source: "npm:skip", displayName: "skip", currentVersion: "1.0.1", latestVersion: "1.1.0" },
    ]);
    expect([...document.querySelectorAll(".session-toast.warning")]
      .filter((toast) => toast.textContent.includes("skip (1.0.1 → 1.1.0)"))).toHaveLength(0);
    ...
    expect(app.isPiPackageUpdateIgnored("pi-package-update-ignore:workspace:ws-1:npm:ws-skip:1.1.0")).toBe(true);
    app.notifyWorkspacePackageUpdateAvailable(
      [{ source: "npm:ws-skip", displayName: "ws-skip", currentVersion: "1.0.1", latestVersion: "1.1.0" }],
      "ws-1",
    );
    expect([...document.querySelectorAll(".session-toast.warning")]
      .filter((toast) => toast.textContent.includes("ws-skip (1.0.1 → 1.1.0)"))).toHaveLength(0);
